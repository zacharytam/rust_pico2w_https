...
static MODEM_TEST_RESULT: embassy_sync::mutex::Mutex<
    embassy_sync::blocking_mutex::raw::CriticalSectionRawMutex,
    heapless::String<2048>,
> = embassy_sync::mutex::Mutex::new(heapless::String::new());

// Signals for button actions
static TEST_MODEM_TRIGGER: embassy_sync::signal::Signal<
    embassy_sync::blocking_mutex::raw::CriticalSectionRawMutex,
    bool,
...
async fn handle_client(socket: &mut TcpSocket<'_>) -> Result<(), embassy_net::tcp::Error> {
    ...
    if let Some(first_line) = request.lines().next() {
        ...
        if parts.len() >= 2 {
            ...
            if path.contains("/test_modem") {
                info!("Test modem triggered!");
                trigger_type = "test_modem";
                    ...
                    "test_modem" => {
                        {
                            ...
                            progress.clear();
                            let _ = progress.push_str("🔄 Testing EC800K connection...");
                        }
                        TEST_MODEM_TRIGGER.signal(true);
                    }
                    "at" => {
                        {
                            let mut progress = ACTION_IN_PROGRESS.lock().await;
                            progress.clear();
                            let _ = progress.push_str("🔄 Sending AT command...");
                        }
            ...
            let test_result = MODEM_TEST_RESULT.lock().await;
...
fn format_html_response(
    ...
    let status_color = if status_display.contains("ERROR") { "red" } 
        else if status_display.contains("Ready") || status_display.contains("OK") || status_display.contains("complete") || status_display.contains("✅") { "green" } 
        else { "orange" };
    
    let test_display = if test_result.is_empty() { "[Run a test to see results]" } else { test_result };
    ...
    let _ = html.push_str("<title>Pico 2W LTE Gateway - EC800K Diagnostics</title>");
    ...
    let _ = html.push_str(".btn-test { background: #4CAF50; color: white; }");
    ...
    let _ = html.push_str(".info-box { background: #e8f4fd; border-left: 4px solid #2196F3; padding: 10px; margin: 15px 0; }");
    let _ = html.push_str("</style>");
    ...
    let _ = html.push_str("<h1>Pico 2W LTE Gateway - EC800K Diagnostics</h1>");
    
    // Information box explaining the system
    let _ = html.push_str("<div class='info-box'>");
    let _ = html.push_str("<strong>System说明：</strong><br>");
    let _ = html.push_str("1. 点击按钮 → 浏览器通过Wi-Fi发送HTTP请求到Pico<br>");
    let _ = html.push_str("2. Pico通过UART向EC800K发送AT命令（波特率: 921600）<br>");
    let _ = html.push_str("3. Pico将AT命令测试结果显示在这个页面上<br>");
    let _ = html.push_str("<small>注意：这是EC800K诊断工具，不是通过LTE访问互联网的工具</small>");
    ...
    let _ = html.push_str(" bytes (Baudrate: 921600)</p>");
    ...
    let _ = html.push_str("<a href='/test_modem'><button class='btn-test'>🔍 Test EC800K Connection</button></a>");
    let _ = html.push_str("<a href='/trigger_at'><button class='btn-at'>📶 Send AT Command</button></a>");
    ...
    // Test Result section
    let _ = html.push_str("<div class='section'>");
    let _ = html.push_str("<h2>EC800K Diagnostics Result</h2>");
    let _ = html.push_str("<pre>");
    let _ = html.push_str(test_display);
    let _ = html.push_str("</pre>");
    let _ = html.push_str("</div>");
    ...
    let _ = html.push_str("<li><b>Baudrate:</b> 921600 (configured in code)</li>");
...
async fn uart_task(mut tx: BufferedUartTx, mut rx: BufferedUartRx) {
    ...
    
    {
        ...
        let _ = data.push_str("=== EC800K LTE Modem Diagnostics ===\n");
        let _ = data.push_str("Pins: GP12(TX)→EC800K_RX, GP13(RX)←EC800K_TX\n");
        let _ = data.push_str("Baud: 921600 (configured)\n");
            ...
            Ok(n) if n > 0 => {
                if let Ok(s) = core::str::from_utf8(&buf[..n]) {
                    ...
                    if s.contains("RDY") || s.contains("READY") {
                        let mut status = EC800K_STATUS.lock().await;
                        status.clear();
                        let _ = status.push_str("Boot complete - Ready for testing");
                    }
                }
            }
            _ => break,
    ...
    
    {
        let mut status = EC800K_STATUS.lock().await;
        if status.is_empty() {
            let _ = status.push_str("Ready - click buttons to test modem");
        }
    }
    ...
    loop {
        ...
        match select(
            select(TEST_MODEM_TRIGGER.wait(), AT_TEST_TRIGGER.wait()),
            ...
            Either::First(trigger_result) => {
                match trigger_result {
                    Either::First(_) => {
                        info!("Test modem triggered!");
                        test_modem_connection(&mut tx, &mut rx).await;
                    }
                    Either::Second(cmd) => {
...
async fn test_modem_connection(tx: &mut BufferedUartTx, rx: &mut BufferedUartRx) {
    info!("Testing modem connection");
    
    {
        let mut status = EC800K_STATUS.lock().await;
        status.clear();
        let _ = status.push_str("Step 1: Sending AT...");
    }
    ...
    if tx.write_all(test_cmd).await.is_err() {
        ...
        let mut status = EC800K_STATUS.lock().await;
        status.clear();
        let _ = status.push_str("❌ Failed: Cannot send AT");
        {
            let mut progress = ACTION_IN_PROGRESS.lock().await;
            progress.clear();
        }
    ...
    
    {
        let mut status = EC800K_STATUS.lock().await;
        status.clear();
        let _ = status.push_str("Step 2: Waiting for response...");
    }
    ...
    {
        let mut test_result = MODEM_TEST_RESULT.lock().await;
        test_result.clear();
        
        if at_response.contains("OK") {
            let _ = test_result.push_str("✅ EC800K is responding to AT commands!\n\n");
            let _ = test_result.push_str("AT response received:\n");
            let _ = test_result.push_str(&at_response);
            let _ = test_result.push_str("\n\nNext steps you can try:\n");
            let _ = test_result.push_str("1. Check signal strength: AT+CSQ\n");
            let _ = test_result.push_str("2. Check network registration: AT+CREG?\n");
            let _ = test_result.push_str("3. Get manufacturer: AT+CGMI\n");
            let _ = test_result.push_str("4. Get model: AT+CGMM\n");
            
            {
                let mut status = EC800K_STATUS.lock().await;
                status.clear();
                let _ = status.push_str("✅ EC800K Ready & Responding");
            }
        } else {
            let _ = test_result.push_str("❌ EC800K not responding to AT commands\n\n");
            let _ = test_result.push_str("Possible issues:\n");
            let _ = test_result.push_str("1. Check wiring connections\n");
            let _ = test_result.push_str("   - Pico GP12 (TX) → EC800K RX\n");
            let _ = test_result.push_str("   - Pico GP13 (RX) ← EC800K TX\n");
            let _ = test_result.push_str("   - GND → GND\n");
            let _ = test_result.push_str("   - 3.3V → VCC (check module voltage requirements)\n");
            let _ = test_result.push_str("2. Verify baudrate matches (921600)\n");
            let _ = test_result.push_str("3. Check EC800K power and boot status\n");
            let _ = test_result.push_str("4. Try resetting the EC800K module\n");
            
            {
                let mut status = EC800K_STATUS.lock().await;
                status.clear();
                let _ = status.push_str("❌ No AT response - Check wiring");
            }
        }
    ...
    info!("Modem connection test completed");
}
...
async fn run_at_test(tx: &mut BufferedUartTx, rx: &mut BufferedUartRx, command: &str) {
    ...
    
    {
        ...
        let mut test_result = MODEM_TEST_RESULT.lock().await;
        test_result.clear();
        let _ = test_result.push_str("AT Command Result:\n");
        let _ = test_result.push_str(&response);
...
async fn main(spawner: Spawner) {
    ...
    uart_config.baudrate = 921600; // 已更新为921600
    ...
    info!("==========================================");
    info!("Pico 2W LTE Gateway - EC800K Diagnostics");
